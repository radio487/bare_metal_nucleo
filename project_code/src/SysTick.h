...
void enable_SysTick(void) {
  ...
  STK_LOAD = 0xFFFFFF
  // Writing anything to this register clears it and stes the COUNTFLAG in STK_CTRL to 0 so we conventionally write zero to it.
  STK_VAL = 0x000000
  ...
  STK_CTRL |= (1 << 2);
  // Manipulating bit 1 we assert that the interrupt will trigger when reasging zero
  STK_CTRL |= (1 << 1);
